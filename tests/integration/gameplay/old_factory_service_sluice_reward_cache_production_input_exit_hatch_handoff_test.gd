## Story226: production service-sluice reward input and exit-hatch handoff.
...
const SERVICE_SLUICE_HATCH: NodePath = (
...
const SERVICE_SLUICE_HATCH_ID: String = (
...
func test_fresh_interact_claims_cache_without_opening_service_exit() -> void:
	var factory: Node = FACTORY_SCENE.instantiate()
	add_child(factory)
	_spawned_nodes.append(factory)
	await _wait_process_frames(2)
	factory.call("set_local_state", _service_sluice_reward_state(false))
	...
	var hatch := factory.get_node_or_null(SERVICE_SLUICE_HATCH) as Node2D
	...
	factory.call("set_local_state", _service_sluice_reward_state(true))
	factory.call("_process", 0.0)
	var stale_cache: Dictionary = factory.call(
		"get_factory_lower_deck_forward_pressure_aftershock_condenser_overflow_pump_runoff_outlet_service_sluice_reward_cache_diagnostics"
	)
	...
	assert_bool(bool(stale_cache.get("claimed", true))).override_failure_message(
		"Held interact from before Story115 became available must remain stale"
	).is_false()
	...
	factory.call("_process", 0.0)
	stale_cache = factory.call(
		"get_factory_lower_deck_forward_pressure_aftershock_condenser_overflow_pump_runoff_outlet_service_sluice_reward_cache_diagnostics"
	)
	assert_bool(bool(cache.call("is_provider_in_reward_range", player))).is_true()
	assert_bool(bool(stale_cache.get("claimed", true))).override_failure_message(
		"No-input placement inside Story115 range must leave the cache unclaimed"
	).is_false()
	...
	assert_bool(claimed).override_failure_message(
		"Story226 requires fresh Input.interact to claim the Story115 cache"
	).is_true()
	if not claimed:
		return
	...
	assert_str(String(reward.get("cache_id", ""))).is_equal(SERVICE_SLUICE_CACHE_ID)
	assert_int(int(reward.get("gears", 0))).is_equal(20)
	assert_str(String(reward.get("source", ""))).is_equal(SERVICE_SLUICE_CACHE_ID)
	...
	assert_str(String(ready_hatch.get("hatch_id", ""))).is_equal(
		SERVICE_SLUICE_HATCH_ID
	)
	...
	assert_bool(bool(locked_tailrace.get("active", true))).is_false()
	assert_bool(bool(locked_tailrace.get("visible", true))).is_false()
	assert_bool(bool(locked_tailrace.get("hazard_contact_active", true))).is_false()
...
func _service_sluice_reward_state(skirmish_cleared: bool) -> Dictionary:
	return {
		...
		"factory_lower_deck_forward_pressure_aftershock_condenser_overflow_pump_runoff_outlet_service_sluice_skirmish_activated": skirmish_cleared,
		"factory_lower_deck_forward_pressure_aftershock_condenser_overflow_pump_runoff_outlet_service_sluice_spark_rat_defeated": skirmish_cleared,
		"factory_lower_deck_forward_pressure_aftershock_condenser_overflow_pump_runoff_outlet_service_sluice_skirmish_cleared": skirmish_cleared,
