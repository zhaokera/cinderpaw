## Story224: production service-hatch input and service-sluice hazard handoff.
...
const SERVICE_SLUICE_VENT: NodePath = (
...
func test_fresh_interact_opens_hatch_then_real_move_crosses_live_service_sluice(
) -> void:
	...
	factory.call("set_local_state", _service_hatch_ready_state())

	var player := factory.get_node_or_null("Player") as PlayerController
	var hatch := factory.get_node_or_null(SERVICE_HATCH) as Node2D
	...
	var vent := factory.get_node_or_null(SERVICE_SLUICE_VENT) as Area2D
	...
	player.global_position = Vector2(9680.0, 456.0)
	...
	# An interaction armed outside the radius must remain stale after entering it.
	...
	assert_bool(bool(held_approach.get("opened", true))).override_failure_message(
		"Holding interact before entering range must not open the service hatch"
	).is_false()
	...
	assert_bool(bool(opened_hatch.get("opened", false))).override_failure_message(
		"Story224 requires production Input.interact to open the Story112 hatch"
	).is_true()

	# Keep downstream assertions independent if the router is the canonical RED.
	if not bool(opened_hatch.get("opened", false)):
		Input.action_release(INTERACT_ACTION)
		factory.call("_process", 0.0)
	...
	var ready_sluice: Dictionary = factory.call(
	...
	assert_float(hatch_visual.position.y).override_failure_message(
		"The opened service hatch must lift clear of the route"
	).is_less_equal(-120.0)
	...
	assert_bool(bool(ready_sluice.get("available", false))).is_true()
	assert_bool(bool(ready_sluice.get("visible", false))).is_true()
	assert_bool(bool(ready_sluice.get("active", true))).override_failure_message(
		"Opening Story112 must not activate Story113 in the same frame"
	).is_false()
	...
	assert_int(int(unlock_vfx.get("spawn_count", 0))).override_failure_message(
		"Repeated fresh interact must not replay hatch unlock feedback"
	).is_equal(1)
	Input.action_release(INTERACT_ACTION)
	factory.call("_process", 0.0)
	...
	for _frame: int in range(3):
		factory.call("_process", 0.0)
	ready_sluice = factory.call(
		"get_factory_lower_deck_forward_pressure_aftershock_condenser_overflow_pump_runoff_outlet_service_sluice_diagnostics"
	)
	assert_bool(bool(ready_sluice.get("active", true))).override_failure_message(
		"Stationary frames after opening must not start Story113"
	).is_false()

	player.set_physics_process(false)
	player.global_position = Vector2(
		float(ready_sluice.get("activation_x", 0.0)) + 4.0,
		456.0
	)
	player.velocity = Vector2.ZERO
	factory.call("_process", 0.0)
	...
	assert_bool(bool(ready_sluice.get("active", true))).override_failure_message(
		"No-input displacement beyond x=10160 must not start Story113"
	).is_false()

	# Re-arm traversal so movement, hazard, and handoff remain independently visible.
	if bool(ready_sluice.get("active", false)):
		factory.call("set_local_state", _service_sluice_ready_state())
		ready_sluice = factory.call(
			"get_factory_lower_deck_forward_pressure_aftershock_condenser_overflow_pump_runoff_outlet_service_sluice_diagnostics"
		)
	player.global_position = Vector2(
		float(ready_sluice.get("activation_x", 0.0)) - 6.0,
		456.0
	)
	player.velocity = Vector2.ZERO
	factory.call("_process", 0.0)
	...
	assert_bool(bool(ready_sluice.get("active", false))).override_failure_message(
		"Fresh production move_right must start the Story113 service sluice"
	).is_true()
	assert_str(String(ready_sluice.get("phase", ""))).is_equal("grace")
	assert_str(String(ready_sluice.get("route_label_text", ""))).is_equal(
	...
	assert_str(String(last_damage.get("source", ""))).is_equal(SERVICE_SLUICE_HAZARD_ID)
	...
	Input.action_release(MOVE_RIGHT_ACTION)
	player.velocity = Vector2.ZERO
	var waiting_skirmish: Dictionary = factory.call(
	...
	assert_bool(bool(safe.get("crossed", false))).is_true()
	assert_bool(bool(safe.get("active", true))).is_false()
	assert_bool(bool(safe.get("hazard_contact_active", true))).is_false()
	assert_bool(bool(waiting_skirmish.get("available", false))).is_true()
	assert_bool(bool(waiting_skirmish.get("active", true))).is_false()
	assert_bool(bool(waiting_skirmish.get("spark_visible", true))).is_false()
	...
	player.set_physics_process(false)
	player.global_position = Vector2(
		float(waiting_skirmish.get("activation_x", 0.0)) + 4.0,
		456.0
	)
	player.velocity = Vector2.ZERO
	factory.call("_process", 0.0)
	waiting_skirmish = factory.call(
		"get_factory_lower_deck_forward_pressure_aftershock_condenser_overflow_pump_runoff_outlet_service_sluice_skirmish_diagnostics"
	)
	assert_bool(bool(waiting_skirmish.get("active", true))).override_failure_message(
		"No-input placement beyond x=10920 must leave Story114 inactive"
	).is_false()
	assert_bool(bool(waiting_skirmish.get("spark_visible", true))).is_false()


func _service_hatch_ready_state() -> Dictionary:
	var state: Dictionary = _service_sluice_ready_state()
	state[
...
func _service_sluice_ready_state() -> Dictionary:
	return {
		...
		"factory_lower_deck_forward_pressure_aftershock_condenser_overflow_pump_runoff_outlet_service_sluice_activated": false,
		"factory_lower_deck_forward_pressure_aftershock_condenser_overflow_pump_runoff_outlet_service_sluice_crossed": false,
