## Player Abilities Story 075: Old Factory lower deck forward-pressure exit gate handoff.
...
const EXIT_GATE_NODE_NAME: String = "FactoryLowerDeckForwardPressureExitGate"
const EXIT_GATE_ID: String = "old_factory_lower_deck_forward_pressure_exit_gate"
const EXIT_GATE_TEXTURE: String = (
...
func test_exit_gate_requires_exit_relay_and_opens_once() -> void:
	assert_bool(FileAccess.file_exists(EXIT_GATE_TEXTURE)).is_true()

	var locked_scene: Node = _factory_scene_with_exit_gate_state(false, false)
	...
	assert_bool(locked_scene.has_method(
		"try_open_factory_lower_deck_forward_pressure_exit_gate"
	)).is_true()
	if (
	...
	assert_bool(bool(locked.get("exit_relay_activated", true))).is_false()
	...
	var destination: Node = _factory_scene_with_exit_gate_state(true, false)
	assert_that(destination).is_not_null()
	if destination == null:
		return

	var player: Node2D = destination.get_node_or_null(FACTORY_PLAYER_NAME) as Node2D
	var exit_gate: Node2D = destination.get_node_or_null(EXIT_GATE_NODE_NAME) as Node2D
	assert_that(player).is_not_null()
	assert_that(exit_gate).is_not_null()
	if player == null or exit_gate == null:
		return
	...
	assert_bool(bool(ready.get("exit_relay_activated", false))).is_true()
	...
	assert_str(String(ready.get("gate_id", ""))).is_equal(EXIT_GATE_ID)
	assert_str(String(ready.get("prompt_text", ""))).is_equal("Open Exit Gate")
	assert_str(String(ready.get("texture_path", ""))).is_equal(EXIT_GATE_TEXTURE)
	assert_bool(bool(ready.get("collision_blocking", false))).is_true()

	player.global_position = exit_gate.global_position
...
func test_exit_gate_restored_open_state_preserves_completed_chain() -> void:
	var restored: Node = _factory_scene_with_exit_gate_state(true, true)
	...
	assert_bool(bool(exit_gate.get("present", false))).is_true()
	assert_bool(bool(exit_gate.get("exit_relay_activated", false))).is_true()
	assert_bool(bool(exit_gate.get("opened", false))).is_true()
	assert_bool(bool(exit_gate.get("available", true))).is_false()
	assert_bool(bool(exit_gate.get("collision_blocking", true))).is_false()
	assert_str(String(exit_gate.get("route_label_text", ""))).is_equal(
		"Forward Pressure Exit Gate Opened"
	)
...
func _factory_scene_with_exit_gate_state(
