## Player Abilities Story 077: Old Factory lower deck forward-pressure beacon ambush.
...
const BEACON_AMBUSH_HAZARD_ID: String = (
...
const REQUIRED_BEACON_AMBUSH_ANIMATIONS: Array[StringName] = [
	&"idle",
	&"run",
	&"attack_tell",
	&"attack",
...
func test_beacon_ambush_requires_route_beacon_lit_and_activates_enemy_hazard() -> void:
	assert_bool(FileAccess.file_exists(STEAM_VENT_TEXTURE)).is_true()

	var locked_scene: Node = _factory_scene_with_beacon_ambush_state(false, false, false)
	...
	assert_bool(locked_scene.has_method(
		"try_activate_factory_lower_deck_forward_pressure_beacon_ambush"
	)).is_true()
	if (
	...
	assert_bool(bool(locked.get("route_marker_lit", true))).is_false()
	...
	var destination: Node = _factory_scene_with_beacon_ambush_state(true, false, false)
	...
	assert_bool(bool(ready.get("route_marker_lit", false))).is_true()
	...
	_assert_beacon_ambush_frame_contract(active)
	assert_bool(bool(active.get("hazard_active", false))).is_true()
	assert_bool(bool(active.get("hazard_visible", false))).is_true()
	assert_str(String(active.get("hazard_id", ""))).is_equal(BEACON_AMBUSH_HAZARD_ID)
	...
	)).is_equal("Clear Forward Pressure Beacon Ambush")
...
func test_beacon_ambush_defeat_persists_without_replaying_route_chain() -> void:
	var destination: Node = _factory_scene_with_beacon_ambush_state(true, false, false)
	assert_that(destination).is_not_null()
	if destination == null:
		return
	...
	if (
		not destination.has_method("try_activate_factory_lower_deck_forward_pressure_beacon_ambush")
		or not destination.has_method(
			"get_factory_lower_deck_forward_pressure_beacon_ambush_diagnostics"
		)
	):
		assert_bool(false).override_failure_message("Story077 beacon ambush APIs missing")
		return
	...
	assert_bool(destination.call(
		...
		999,
		{"source": &"unit_test_forward_pressure_beacon_ambush"}
	)).is_true()
	...
	var restored: Node = _factory_scene_with_beacon_ambush_state(true, true, true)
...
func _factory_scene_with_beacon_ambush_state(
		route_marker_lit: bool,
		beacon_ambush_activated: bool,
		beacon_ambush_defeated: bool
) -> Node:
	var destination: Node = _instantiate_factory_scene()
	if destination == null:
		return null
	destination.call("set_local_state", _forward_pressure_route_beacon_base_state().merged({
		"factory_lower_deck_forward_pressure_route_handoff_marker_lit": route_marker_lit,
		"factory_lower_deck_forward_pressure_beacon_ambush_activated": (
			beacon_ambush_activated
		),
		"factory_lower_deck_forward_pressure_beacon_ambush_defeated": (
...
func _forward_pressure_route_beacon_base_state() -> Dictionary:
	return {
...
func _assert_beacon_ambush_frame_contract(diagnostics: Dictionary) -> void:
	var frame_counts: Dictionary = diagnostics.get("animation_frame_counts", {}) as Dictionary
	for animation_name: StringName in REQUIRED_BEACON_AMBUSH_ANIMATIONS:
