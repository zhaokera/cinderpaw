## Player Abilities Story 087: Old Factory lower-deck aftershock exhaust pursuer.
...
const EXHAUST_PURSUER_OPENING_GRACE_FRAMES: int = 10
...
func test_aftershock_exhaust_pursuer_requires_exhaust_crossed_and_activates() -> void:
	var locked_scene: Node = _factory_scene_with_aftershock_exhaust_pursuer_state(
		false,
		false
	)
	...
	assert_bool(bool(locked.get("aftershock_exhaust_crossed", true))).is_false()
	...
	var destination: Node = _factory_scene_with_aftershock_exhaust_pursuer_state(
		true,
		false
	)
	...
	assert_bool(bool(ready.get("aftershock_exhaust_crossed", false))).is_true()
	...
	assert_int(int(active.get("coil_entity_id", 0))).is_equal(EXHAUST_PURSUER_ENTITY_ID)
	...
	_assert_pursuer_pacing_contract(active)


func test_aftershock_exhaust_pursuer_defeat_persists_without_replaying_chain() -> void:
	var destination: Node = _factory_scene_with_aftershock_exhaust_pursuer_state(
		true,
		false
	)
	...
	assert_bool(destination.call("apply_damage", EXHAUST_PURSUER_ENTITY_ID, 999, {
		"source": &"unit_test_aftershock_exhaust_pursuer_coil",
	})).is_true()
	...
	var restored: Node = _factory_scene_with_aftershock_exhaust_pursuer_state(
		true,
		true
	)
	...
	assert_bool(bool(restored_pursuer.get("present", false))).is_true()
	assert_bool(bool(restored_pursuer.get("aftershock_exhaust_crossed", false))).is_true()
	assert_bool(bool(restored_pursuer.get("active", true))).is_false()
	assert_bool(bool(restored_pursuer.get("cleared", false))).is_true()
	assert_bool(bool(restored_pursuer.get("coil_visible", true))).is_false()
	assert_str(String(restored_pursuer.get("route_label_text", ""))).is_equal(
		"Forward Pressure Exhaust Pursuer Cleared"
	)
...
func _factory_scene_with_aftershock_exhaust_pursuer_state(
	...
	if destination == null:
		return null
	destination.call("set_local_state", _aftershock_exhaust_pursuer_base_state().merged({
...
func _aftershock_exhaust_pursuer_base_state() -> Dictionary:
	return {
...
func _assert_pursuer_pacing_contract(diagnostics: Dictionary) -> void:
	...
	assert_int(int(pacing.get("coil_opening_grace_frames", 0))).is_equal(
		EXHAUST_PURSUER_OPENING_GRACE_FRAMES
	)
