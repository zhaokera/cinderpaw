## Story198: forward-pressure overrun production combat handoff.
...
func test_real_attack_clears_overrun_before_fresh_move_starts_breaker() -> void:
	var factory: Node = _instantiate_factory_scene()
	assert_that(factory).is_not_null()
	if factory == null:
		return
	await _wait_process_frames(18)
	factory.call("set_local_state", _overrun_ready_state())

	var player := factory.get_node_or_null("Player") as PlayerController
	var enemy := factory.get_node_or_null(
		"FactoryLowerDeckForwardPressureOverrunSparkRat"
	) as CharacterBody2D
	var breaker_enemy := factory.get_node_or_null(
		"FactoryLowerDeckForwardPressureBreakerSparkRat"
	) as CharacterBody2D
	assert_that(player).is_not_null()
	assert_that(enemy).is_not_null()
	assert_that(breaker_enemy).is_not_null()
	if player == null or enemy == null or breaker_enemy == null:
		return

	player.global_position = Vector2(1614.0, enemy.global_position.y)
	...
	assert_bool(bool(active.get("active", false))).override_failure_message(
		"Story198 requires real forward movement to enter the Story078 fight"
	).is_true()
	assert_int(int(active.get("entity_id", 0))).is_equal(OVERRUN_ENTITY_ID)
	assert_int(int(enemy.call("get_entity_id"))).is_equal(OVERRUN_ENTITY_ID)
	...
	assert_bool(bool(waiting_breaker.get("available", true))).is_false()
	assert_bool(bool(waiting_breaker.get("active", true))).is_false()
	assert_bool(bool(waiting_breaker.get("enemy_visible", true))).is_false()
	assert_bool(bool(waiting_breaker.get("hazard_active", true))).is_false()
	var breaker_collision := breaker_enemy.call(
	...
	assert_str(String(breaker_collision.get_hurtbox_state())).override_failure_message(
		"Inactive entity 2123 must not participate in Story078 player attacks"
	...
	assert_bool(bool(factory.call(
		...
		OVERRUN_ENTITY_ID,
		12,
		{"source": &"story198_nonlethal_setup"}
	))).is_true()
	...
	player.global_position = Vector2(1704.0, enemy.global_position.y)
	player.velocity = Vector2.ZERO
	...
	assert_bool(defeated_with_input).override_failure_message(
		"Story198 requires Input.attack to deliver the lethal hit to entity 2122"
	).is_true()
	if not defeated_with_input:
		return
	assert_float(player.global_position.x).is_greater_equal(1668.0)
	assert_int(int(enemy.call("get_current_hp"))).is_equal(0)
	var lethal_hit: Dictionary = factory.call("get_last_player_hit_metadata")
	assert_int(int(lethal_hit.get("target_id", 0))).is_equal(OVERRUN_ENTITY_ID)
	...
	# Model rightward residual movement from the lethal tick; the clear frame must win.
	...
	assert_bool(bool(waiting_breaker.get("active", true))).override_failure_message(
		"Story079 must wait for fresh rightward movement after the Story078 clear"
	).is_false()
	assert_bool(bool(waiting_breaker.get("enemy_visible", true))).is_false()
	assert_bool(bool(waiting_breaker.get("hazard_active", true))).is_false()
	...
	for _frame: int in range(16):
		await get_tree().physics_frame
		var breaker_probe: Dictionary = factory.call(
			"get_factory_lower_deck_forward_pressure_breaker_stand_diagnostics"
		)
		if bool(breaker_probe.get("active", false)):
			break
	Input.action_release(MOVE_RIGHT_ACTION)
	var advanced_breaker: Dictionary = factory.call(
		"get_factory_lower_deck_forward_pressure_breaker_stand_diagnostics"
	)
	assert_float(player.global_position.x).is_greater(stationary_x)
	assert_bool(bool(advanced_breaker.get("active", false))).is_true()
	assert_int(int(advanced_breaker.get("entity_id", 0))).is_equal(BREAKER_ENTITY_ID)
	assert_bool(bool(advanced_breaker.get("enemy_visible", false))).is_true()
	assert_bool(bool(advanced_breaker.get("hazard_active", false))).is_true()
	assert_bool(bool(advanced_breaker.get("breaker_visible", true))).is_false()
	assert_str(String(advanced_breaker.get("route_label_text", ""))).is_equal(
...
func _defeat_enemy_with_real_attack_input(
	...
	for _attempt: int in range(2):
		if _is_overrun_defeated(factory):
			return true
		...
		for _frame: int in range(16):
			...
			if _is_overrun_defeated(factory):
				return true
	return _is_overrun_defeated(factory)


func _is_overrun_defeated(factory: Node) -> bool:
...
func _overrun_ready_state() -> Dictionary:
	return {
