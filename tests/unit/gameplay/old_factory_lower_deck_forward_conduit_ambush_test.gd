...
const REQUIRED_FORWARD_CONDUIT_ANIMATIONS: Array[StringName] = [
	&"idle",
	&"run",
	&"attack_tell",
	&"attack",
...
func test_forward_conduit_ambush_requires_opened_forward_hatch_and_keeps_lift_optional() -> void:
	var locked_scene: Node = _factory_scene_with_forward_hatch_opened(false)
	...
	if (
		not locked_scene.has_method("get_factory_lower_deck_forward_conduit_diagnostics")
		or not locked_scene.has_method("try_activate_factory_lower_deck_forward_conduit")
	):
		return
	...
	assert_str(String(active.get("hazard_id", ""))).is_equal(FORWARD_CONDUIT_HAZARD_ID)
	assert_int(int(active.get("hazard_damage", 0))).is_equal(8)
	assert_float(float(active.get("hazard_cooldown_sec", 0.0))).is_equal(1.0)
	_assert_forward_conduit_frame_contract(active)

	var objective: Dictionary = destination.call("get_factory_route_objective_diagnostics")
	assert_str(String(objective.get("objective_id", ""))).is_equal("clear_forward_conduit_ambush")
...
func test_forward_conduit_ambush_clear_persists_without_replaying_relay_forward_chain() -> void:
	var destination: Node = _factory_scene_with_forward_hatch_opened(true)
	assert_that(destination).is_not_null()
	if destination == null:
		return
	...
	assert_bool(destination.has_method("get_factory_lower_deck_forward_conduit_diagnostics")).is_true()
	if (
		not destination.has_method("try_activate_factory_lower_deck_forward_conduit")
		or not destination.has_method("get_factory_lower_deck_forward_conduit_diagnostics")
	):
		return
	...
	assert_bool(destination.call(
		...
		999,
		{"source": &"unit_test_forward_conduit"}
	)).is_true()
	...
	assert_bool(bool(local_state.get("factory_lower_deck_forward_conduit_activated", false))).is_true()
	assert_bool(bool(local_state.get("factory_lower_deck_forward_conduit_defeated", false))).is_true()
...
func _assert_forward_conduit_frame_contract(diagnostics: Dictionary) -> void:
	var frame_counts: Dictionary = diagnostics.get("animation_frame_counts", {}) as Dictionary
	for animation_name: StringName in REQUIRED_FORWARD_CONDUIT_ANIMATIONS:
