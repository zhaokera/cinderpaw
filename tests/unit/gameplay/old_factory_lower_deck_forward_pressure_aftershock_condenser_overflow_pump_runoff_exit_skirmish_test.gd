## Player Abilities Story 108: Old Factory runoff duct exit skirmish.
...
const RUNOFF_EXIT_COIL_RAT_NODE_NAME: String = (
	"FactoryLowerDeckForwardPressureAftershockCondenserOverflowPumpRunoffExitCoilRat"
)
const RUNOFF_EXIT_COIL_RAT_ENTITY_ID: int = 2140
const RUNOFF_EXIT_ACTIVATION_X: float = 7800.0
const RUNOFF_EXIT_OPENING_GRACE_FRAMES: int = 10
...
func test_runoff_exit_skirmish_requires_runoff_duct_crossed_and_activates_coil_contract(
) -> void:
	assert_bool(FileAccess.file_exists(COIL_RAT_SPRITE_FRAMES)).is_true()

	var locked_scene: Node = _factory_scene_with_runoff_exit_state(false, false)
	...
	assert_bool(bool(locked.get("runoff_duct_crossed", true))).is_false()
	...
	var destination: Node = _factory_scene_with_runoff_exit_state(true, false)
	...
	assert_bool(bool(ready.get("runoff_duct_crossed", false))).is_true()
	...
	assert_str(String(ready.get("coil_node_name", ""))).is_equal(
		RUNOFF_EXIT_COIL_RAT_NODE_NAME
	)
	assert_float(float(ready.get("ground_width", 0.0))).is_greater_equal(8320.0)
	assert_float(float(ready.get("right_wall_x", 0.0))).is_greater_equal(8300.0)
	assert_int(int(ready.get("camera_limit_right", 0))).is_greater_equal(8320)
	assert_float(float(ready.get("activation_x", 0.0))).is_equal(
		RUNOFF_EXIT_ACTIVATION_X
	)
	assert_str(String(ready.get("route_label_text", ""))).is_equal(
		"Overflow Pump Runoff Duct Crossed"
	)

	player.global_position.x = RUNOFF_EXIT_ACTIVATION_X - 4.0
	...
	assert_int(int(active.get("coil_entity_id", 0))).is_equal(
		RUNOFF_EXIT_COIL_RAT_ENTITY_ID
	)
	...
	_assert_runoff_exit_pacing_contract(active)


func test_runoff_exit_skirmish_defeat_persists_without_replaying_runoff_duct(
) -> void:
	var destination: Node = _factory_scene_with_runoff_exit_state(true, false)
	assert_that(destination).is_not_null()
	if destination == null:
	...
	assert_bool(destination.call("apply_damage", RUNOFF_EXIT_COIL_RAT_ENTITY_ID, 999, {
		"source": &"unit_test_runoff_exit_skirmish_coil",
	})).is_true()
	...
	assert_bool(bool(cleared.get("coil_visible", false))).override_failure_message(
		"A live defeat keeps the three-frame Coil Rat death presentation visible"
	).is_true()
	...
	var restored: Node = _factory_scene_with_runoff_exit_state(true, true)
...
func _factory_scene_with_runoff_exit_state(
	...
	if destination == null:
		return null
	destination.call("set_local_state", _runoff_exit_base_state().merged({
...
func _runoff_exit_base_state() -> Dictionary:
	return {
...
func _assert_coil_rat_frame_contract(diagnostics: Dictionary) -> void:
	var frame_counts: Dictionary = diagnostics.get("coil_animation_frame_counts", {}) as Dictionary
...
func _assert_runoff_exit_pacing_contract(diagnostics: Dictionary) -> void:
	...
	assert_int(int(pacing.get("opening_grace_total_frames", 0))).is_equal(
		RUNOFF_EXIT_OPENING_GRACE_FRAMES
	)
