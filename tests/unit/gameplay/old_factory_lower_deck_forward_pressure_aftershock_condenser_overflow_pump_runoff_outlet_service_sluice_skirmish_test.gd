## Player Abilities Story 114: Old Factory service sluice Spark Rat skirmish.
extends GdUnitTestSuite

const FACTORY_SCENE_PATH: String = "res://scenes/factory_route_transition_shell.tscn"
const FACTORY_PLAYER_NAME: String = "Player"
const SERVICE_SLUICE_SPARK_RAT_NODE_NAME: String = (
	"FactoryLowerDeckForwardPressureAftershockCondenserOverflowPumpRunoffOutletServiceSluiceSparkRat"
)
const SERVICE_SLUICE_SPARK_RAT_ENTITY_ID: int = 2142
const SERVICE_SLUICE_SPARK_RAT_ACTIVATION_X: float = 10920.0
const SERVICE_SLUICE_SPARK_RAT_OPENING_GRACE_FRAMES: int = 12
...
func test_service_sluice_skirmish_requires_sluice_crossed_and_activates_spark_contract(
) -> void:
	assert_bool(FileAccess.file_exists(SPARK_RAT_SPRITE_FRAMES)).is_true()

	var locked_scene: Node = _factory_scene_with_service_sluice_spark_state(false, false)
	...
	assert_bool(bool(locked.get("service_sluice_crossed", true))).is_false()
	...
	var destination: Node = _factory_scene_with_service_sluice_spark_state(true, false)
	...
	assert_bool(bool(ready.get("service_sluice_crossed", false))).is_true()
	...
	assert_str(String(ready.get("spark_node_name", ""))).is_equal(
		SERVICE_SLUICE_SPARK_RAT_NODE_NAME
	)
	assert_float(float(ready.get("activation_x", 0.0))).is_equal(
		SERVICE_SLUICE_SPARK_RAT_ACTIVATION_X
	)
	assert_float(float(ready.get("right_wall_x", 0.0))).is_greater_equal(11500.0)
	assert_int(int(ready.get("camera_limit_right", 0))).is_greater_equal(11520)
	assert_float(float(ready.get("background_width", 0.0))).is_greater_equal(11520.0)
	assert_float(float(ready.get("ground_right_edge_x", 0.0))).is_greater_equal(11520.0)
	assert_str(String(ready.get("route_label_text", ""))).is_equal(
		"Runoff Outlet Service Sluice Crossed"
	)

	player.global_position.x = SERVICE_SLUICE_SPARK_RAT_ACTIVATION_X - 4.0
	...
	assert_int(int(active.get("spark_entity_id", 0))).is_equal(
		SERVICE_SLUICE_SPARK_RAT_ENTITY_ID
	)
...
func test_service_sluice_skirmish_defeat_persists_and_backfills_sluice_chain(
) -> void:
	var destination: Node = _factory_scene_with_service_sluice_spark_state(true, false)
	assert_that(destination).is_not_null()
	if destination == null:
	...
	assert_bool(destination.call("apply_damage", SERVICE_SLUICE_SPARK_RAT_ENTITY_ID, 999, {
		"source": &"unit_test_service_sluice_spark_rat",
	})).is_true()
	...
	var restored: Node = _factory_scene_with_service_sluice_spark_state(false, true)
...
func _factory_scene_with_service_sluice_spark_state(
		service_sluice_crossed: bool,
		service_sluice_spark_cleared: bool
) -> Node:
...
func _assert_spark_rat_pacing_contract(diagnostics: Dictionary) -> void:
	...
	assert_int(int(pacing.get("opening_grace_total_frames", 0))).is_equal(
		SERVICE_SLUICE_SPARK_RAT_OPENING_GRACE_FRAMES
	)
