...
func test_fish_bone_t1b_spends_and_increases_only_fish_bone_runtime_damage() -> void:
	...
	if not bool(skill_tree.call("has_skill_definition", FISH_BONE_T1B_SKILL_ID)):
		return
	...
	assert_str(String(hud.call("get_active_skill_tree_entry_id"))).is_equal(
		String(FISH_BONE_T1B_SKILL_ID)
	)
	assert_str(String(hud.call("get_menu_subtitle"))).contains("Fish Bone T1")
	assert_str(String(hud.call("get_menu_subtitle"))).contains("8% more damage")
	...
	assert_bool(bool(upgraded_scene.call("has_skill", FISH_BONE_T1B_SKILL_ID))).is_true()
	assert_float(float(skill_tree.call("get_stat_bonus", &"damage"))).is_equal_approx(
		FISH_BONE_DAMAGE_BONUS,
		0.0001
	)
	...
	assert_float(float(upgraded_modifiers.get("skill_weapon_bonus", 0.0))).is_equal_approx(
		0.0,
		0.0001
	)
	assert_int(int(baseline_hit.get("base_damage", 0))).is_equal(40)
	assert_int(int(upgraded_hit.get("base_damage", 0))).is_equal(40)
	assert_float(float(baseline_hit.get("attack_damage", 0.0))).is_equal_approx(100.0, 0.001)
	assert_float(float(upgraded_hit.get("attack_damage", 0.0))).is_equal_approx(108.0, 0.001)
	assert_int(int(baseline_hit.get("final_damage", 0))).is_equal(100)
	assert_int(int(upgraded_hit.get("final_damage", 0))).is_equal(108)
	assert_int(int(baseline_scene.get_node("Enemy").call("get_current_hp"))).is_equal(200)
	assert_int(int(upgraded_scene.get_node("Enemy").call("get_current_hp"))).is_equal(192)
