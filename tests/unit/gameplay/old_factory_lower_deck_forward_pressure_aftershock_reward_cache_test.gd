## Player Abilities Story 084: Old Factory lower-deck forward-pressure aftershock reward cache.
...
const AFTERSHOCK_REWARD_CACHE_ID: String = (
	"old_factory_lower_deck_forward_pressure_aftershock_reward_cache"
)
const AFTERSHOCK_REWARD_CACHE_TEXTURE: String = (
...
func test_aftershock_reward_cache_requires_aftershock_clear_and_keeps_lift_optional() -> void:
	var locked_scene: Node = _factory_scene_with_aftershock_reward_state(false, false)
	...
	assert_bool(bool(locked_cache.get("coil_aftershock_cleared", true))).is_false()
	...
	var destination: Node = _factory_scene_with_aftershock_reward_state(true, false)
	...
	assert_bool(bool(available_cache.get("coil_aftershock_cleared", false))).is_true()
	...
	)).is_equal("Forward Pressure Coil Aftershock Cleared")
...
func test_aftershock_reward_cache_claim_persists_without_replaying_forward_chain() -> void:
	var destination: Node = _factory_scene_with_aftershock_reward_state(true, false)
	assert_that(destination).is_not_null()
	if destination == null:
		return
	...
	assert_str(String(reward.get("cache_id", ""))).is_equal(AFTERSHOCK_REWARD_CACHE_ID)
	assert_int(int(reward.get("gears", 0))).is_equal(20)
	assert_str(String(reward.get("source", ""))).is_equal(AFTERSHOCK_REWARD_CACHE_ID)
...
func test_aftershock_reward_cache_unlocks_on_live_aftershock_defeat() -> void:
	var destination: Node = _factory_scene_with_aftershock_reward_state(false, false)
	assert_that(destination).is_not_null()
	if destination == null:
		return
	...
	var ready_aftershock: Dictionary = destination.call(
		"get_factory_lower_deck_forward_pressure_coil_aftershock_diagnostics"
	)
	assert_bool(bool(ready_aftershock.get("available", false))).is_true()
	assert_bool(bool(ready_aftershock.get("cleared", true))).is_false()
	player.global_position.x = float(ready_aftershock.get("activation_x", 0.0)) + 4.0
	assert_bool(bool(destination.call(
	...
	assert_bool(destination.call("apply_damage", AFTERSHOCK_COIL_RAT_ENTITY_ID, 999, {
		"source": &"unit_test_aftershock_reward_cache_unlock",
	})).is_true()
...
func _factory_scene_with_aftershock_reward_state(
		aftershock_cleared: bool,
		cache_claimed: bool
) -> Node:
	var destination: Node = _instantiate_factory_scene()
	if destination == null:
		return null
	destination.call("set_local_state", _aftershock_reward_base_state().merged({
...
func _aftershock_reward_base_state() -> Dictionary:
	return {
