## Player Abilities Story 083: Old Factory lower deck forward-pressure Coil Aftershock.
...
func test_coil_aftershock_requires_pincer_clear_and_activates_runtime_contract() -> void:
	var locked_scene: Node = _factory_scene_with_coil_aftershock_state(false, false)
	...
	assert_bool(locked_scene.has_method(
		"try_activate_factory_lower_deck_forward_pressure_coil_aftershock"
	)).is_true()
	if (
	...
	assert_bool(bool(locked.get("coil_pincer_cleared", true))).is_false()
	...
	var destination: Node = _factory_scene_with_coil_aftershock_state(true, false)
	...
	assert_bool(bool(ready.get("coil_pincer_cleared", false))).is_true()
	...
	assert_int(int(active.get("coil_entity_id", 0))).is_equal(AFTERSHOCK_COIL_RAT_ENTITY_ID)
	assert_str(String(active.get("coil_family_id", ""))).is_equal("factory_coil_rat")
	assert_str(String(active.get("route_label_text", ""))).is_equal("Contain Coil Aftershock")
	_assert_aftershock_frame_contract(active)
	_assert_aftershock_pacing_contract(active)


func test_coil_aftershock_defeat_persists_without_replaying_route_chain() -> void:
	var destination: Node = _factory_scene_with_coil_aftershock_state(true, false)
	assert_that(destination).is_not_null()
	if destination == null:
		return
	...
	assert_bool(destination.call("apply_damage", AFTERSHOCK_COIL_RAT_ENTITY_ID, 999, {
		"source": &"unit_test_forward_pressure_coil_aftershock_coil",
	})).is_true()
	...
	var restored: Node = _factory_scene_with_coil_aftershock_state(true, true)
	...
	assert_bool(bool(restored_aftershock.get("present", false))).is_true()
	assert_bool(bool(restored_aftershock.get("coil_pincer_cleared", false))).is_true()
	assert_bool(bool(restored_aftershock.get("active", true))).is_false()
	assert_bool(bool(restored_aftershock.get("cleared", false))).is_true()
	assert_bool(bool(restored_aftershock.get("coil_visible", true))).is_false()
	assert_str(String(restored_aftershock.get("route_label_text", ""))).is_equal(
		"Forward Pressure Coil Aftershock Cleared"
	)
...
func _factory_scene_with_coil_aftershock_state(
		pincer_cleared: bool,
		coil_defeated: bool
) -> Node:
	...
	if destination == null:
		return null
	destination.call("set_local_state", _forward_pressure_aftershock_base_state().merged({
...
func _forward_pressure_aftershock_base_state() -> Dictionary:
	return {
...
func _assert_aftershock_frame_contract(diagnostics: Dictionary) -> void:
	...
	for animation_name: StringName in REQUIRED_AFTERSHOCK_ANIMATIONS:
		assert_int(int(coil_counts.get(String(animation_name), 0))).is_greater_equal(
			MIN_CHARACTER_ANIMATION_FRAMES
		)


func _assert_aftershock_pacing_contract(diagnostics: Dictionary) -> void:
	...
	assert_int(int(pacing.get("opening_grace_frames", 0))).is_equal(
		AFTERSHOCK_COIL_RAT_OPENING_GRACE_FRAMES
	)
