## Story221: production runoff-exit combat and reward-cache handoff.
...
const RUNOFF_EXIT_ENTITY_ID: int = 2140
const PLAYER_LIGHT_HITBOX_ID: StringName = &"cat_claw_light"
const RUNOFF_EXIT_COIL: NodePath = (
...
const RUNOFF_EXIT_CACHE: NodePath = (
...
const RUNOFF_EXIT_CACHE_ID: String = (
...
func test_real_move_and_attack_reveal_then_fresh_interact_claims_runoff_cache(
) -> void:
	var factory: Node = FACTORY_SCENE.instantiate()
	add_child(factory)
	_spawned_nodes.append(factory)
	await _wait_process_frames(2)
	factory.call("set_local_state", _runoff_exit_ready_state())

	var player := factory.get_node_or_null("Player") as PlayerController
	var coil := factory.get_node_or_null(RUNOFF_EXIT_COIL) as CharacterBody2D
	var duct := factory.get_node_or_null(RUNOFF_DUCT) as Sprite2D
	var cache := factory.get_node_or_null(RUNOFF_EXIT_CACHE) as Node2D
	...
	if player == null or coil == null or duct == null or cache == null or gate == null:
		return
	...
	assert_bool(bool(waiting.get("active", true))).override_failure_message(
		"No-input displacement beyond x=7800 must not activate Story108"
	).is_false()
	...
	assert_bool(bool(active.get("active", false))).override_failure_message(
		"Story221 requires production move_right to activate entity 2140"
	).is_true()
	assert_int(int(active.get("coil_entity_id", 0))).is_equal(RUNOFF_EXIT_ENTITY_ID)
	...
	assert_int(coil.z_index).override_failure_message(
		"The active Coil Rat must render in front of the runoff duct"
	...
	assert_bool(bool(factory.call(
		"apply_damage",
		RUNOFF_EXIT_ENTITY_ID,
		12,
		{"source": &"story221_nonlethal_setup"}
	))).is_true()
	assert_int(int(coil.call("get_current_hp"))).is_equal(12)

	coil.global_position = Vector2(7908.0, 482.0)
	player.global_position = Vector2(7940.0, 482.0)
	player.velocity = Vector2.ZERO
	...
	assert_bool(await _hit_with_real_attack_input(
		...
		"Story221 requires Input.attack to finish entity 2140"
	).is_true()

	var hit: Dictionary = factory.call("get_last_player_hit_metadata")
	assert_int(int(hit.get("target_id", 0))).is_equal(RUNOFF_EXIT_ENTITY_ID)
	...
	var immediate_gate: Dictionary = factory.call(
	...
	assert_bool(bool(immediate_gate.get("visible", true))).is_false()
	...
	assert_bool(bool(cleared.get("coil_visible", false))).override_failure_message(
		"Story221 must preserve the three-frame Coil Rat death presentation"
	).is_true()
	...
	assert_bool(bool(available_cache.get("claimed", true))).override_failure_message(
		"Held pre-clear interact must not consume the newly revealed Story109 cache"
	).is_false()
	assert_str(String(available_cache.get("cache_id", ""))).is_equal(
		RUNOFF_EXIT_CACHE_ID
	)
	...
	assert_bool(bool(available_cache.get("claimed", true))).override_failure_message(
		"No-input displacement into reward range must leave Story109 unclaimed"
	).is_false()
	...
	assert_bool(claimed).override_failure_message(
		"Story221 requires fresh Input.interact to claim the Story109 cache"
	).is_true()
	if not claimed:
		return
	...
	assert_str(String(reward.get("source", ""))).is_equal(RUNOFF_EXIT_CACHE_ID)
	...
	var available_gate: Dictionary = factory.call(
		"get_factory_lower_deck_forward_pressure_aftershock_condenser_overflow_pump_runoff_exit_gate_diagnostics"
	)
	assert_bool(bool(available_gate.get("available", false))).is_true()
	assert_bool(bool(available_gate.get("visible", false))).is_true()
	assert_bool(bool(available_gate.get("opened", true))).is_false()
	assert_bool(bool(available_gate.get("collision_blocking", false))).is_true()
	assert_str(String(available_gate.get("prompt_text", ""))).is_equal(
		"Open Runoff Exit Gate"
	)
...
func _runoff_exit_ready_state() -> Dictionary:
	return {
		...
		"factory_lower_deck_forward_pressure_aftershock_condenser_overflow_pump_runoff_exit_skirmish_cleared": false,
		"factory_lower_deck_forward_pressure_aftershock_condenser_overflow_pump_runoff_exit_reward_cache_claimed": false,
		"factory_lower_deck_forward_pressure_aftershock_condenser_overflow_pump_runoff_exit_gate_opened": false,
