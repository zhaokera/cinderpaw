## Story161: Long Tail T1-B is a weapon-scoped, player-visible damage choice.
extends GdUnitTestSuite

const MAIN_SCENE: PackedScene = preload("res://scenes/main.tscn")
const LONG_TAIL_T1B_SKILL_ID: StringName = &"long_tail_t1b"
const LONG_TAIL_WEAPON_ID: StringName = &"long_tail"
const LONG_TAIL_DAMAGE_BONUS: float = 0.05
...
func test_long_tail_t1b_spends_and_increases_only_long_tail_runtime_damage() -> void:
	...
	if not bool(skill_tree.call("has_skill_definition", LONG_TAIL_T1B_SKILL_ID)):
		return
	...
	assert_bool(bool(upgraded_scene.call("has_skill", LONG_TAIL_T1B_SKILL_ID))).is_true()
	assert_float(float(skill_tree.call("get_stat_bonus", &"damage"))).is_equal_approx(
		LONG_TAIL_DAMAGE_BONUS,
		0.0001
	)
	...
	assert_float(float(upgraded_hit.get("attack_damage", 0.0))).is_equal_approx(
		float(baseline_hit.get("attack_damage", 0.0)) * (1.0 + LONG_TAIL_DAMAGE_BONUS),
		0.001
	)
	assert_int(int(baseline_hit.get("final_damage", 0))).is_equal(37)
	assert_int(int(upgraded_hit.get("final_damage", 0))).is_equal(39)
