## Player Abilities Story 091: Old Factory aftershock exhaust escape skirmish.
...
const ESCAPE_SPARK_RAT_ENTITY_ID: int = 2134
const ESCAPE_COIL_RAT_ENTITY_ID: int = 2135
const ESCAPE_ACTIVATION_X: float = 3112.0
const ESCAPE_SPARK_RAT_OPENING_GRACE_FRAMES: int = 10
const ESCAPE_COIL_RAT_OPENING_GRACE_FRAMES: int = 22
...
const REQUIRED_ESCAPE_SKIRMISH_ANIMATIONS: Array[StringName] = [
	&"idle",
	&"run",
	&"attack_tell",
	&"attack",
...
func test_escape_skirmish_requires_breaker_cut_and_activates_dual_contract() -> void:
	...
	var locked_scene: Node = _factory_scene_with_escape_skirmish_state(
		false,
		false,
		false
	)
	...
	var destination: Node = _factory_scene_with_escape_skirmish_state(
		true,
		false,
		false
	)
	...
	assert_float(float(ready.get("activation_x", 0.0))).is_equal(ESCAPE_ACTIVATION_X)
	assert_str(String(ready.get("route_label_text", ""))).is_equal(
		"Aftershock Exhaust Pressure Cut"
	)

	player.global_position.x = ESCAPE_ACTIVATION_X - 4.0
	...
	assert_int(int(active.get("spark_entity_id", 0))).is_equal(
		ESCAPE_SPARK_RAT_ENTITY_ID
	)
	assert_int(int(active.get("coil_entity_id", 0))).is_equal(
		ESCAPE_COIL_RAT_ENTITY_ID
	)
	...
	assert_str(String(active.get("route_label_text", ""))).is_equal(
		"Break Aftershock Exhaust Escape"
	)
	_assert_escape_skirmish_frame_contract(active)
	_assert_escape_skirmish_pacing_contract(active)


func test_escape_skirmish_defeat_persists_without_replaying_breaker_chain() -> void:
	var destination: Node = _factory_scene_with_escape_skirmish_state(
		true,
		false,
		false
	...
	assert_bool(destination.call("apply_damage", ESCAPE_SPARK_RAT_ENTITY_ID, 999, {
		"source": &"unit_test_aftershock_exhaust_escape_spark",
	})).is_true()
	...
	assert_bool(destination.call("apply_damage", ESCAPE_COIL_RAT_ENTITY_ID, 999, {
		"source": &"unit_test_aftershock_exhaust_escape_coil",
	})).is_true()
	await get_tree().process_frame
	...
	var resynced: Dictionary = destination.call(
		"get_factory_lower_deck_forward_pressure_aftershock_exhaust_escape_skirmish_diagnostics"
	)
	assert_bool(bool(resynced.get("cleared", false))).is_true()
	assert_bool(bool(resynced.get("spark_visible", true))).is_false()
	assert_bool(bool(resynced.get("coil_visible", true))).is_false()

	var restored: Node = _factory_scene_with_escape_skirmish_state(
		true,
		true,
		true
	)
	...
	assert_bool(bool(restored_escape.get("present", false))).is_true()
	assert_bool(bool(restored_escape.get("breaker_cut", false))).is_true()
	assert_bool(bool(restored_escape.get("active", true))).is_false()
...
func _factory_scene_with_escape_skirmish_state(
		breaker_cut: bool,
		spark_defeated: bool,
		coil_defeated: bool
) -> Node:
	...
	if destination == null:
		return null
	destination.call("set_local_state", _escape_base_state().merged({
		...
		"factory_lower_deck_forward_pressure_aftershock_exhaust_escape_skirmish_activated": (
			spark_defeated or coil_defeated
		),
		"factory_lower_deck_forward_pressure_aftershock_exhaust_escape_skirmish_spark_rat_defeated": (
			spark_defeated
		),
		...
		"factory_lower_deck_forward_pressure_aftershock_exhaust_escape_skirmish_cleared": (
			spark_defeated and coil_defeated
		),
	}, true))
	return destination
...
func _escape_base_state() -> Dictionary:
	return {
...
func _assert_escape_skirmish_frame_contract(diagnostics: Dictionary) -> void:
	var spark_frame_counts: Dictionary = diagnostics.get(
		"spark_animation_frame_counts",
		{}
	) as Dictionary
	var coil_frame_counts: Dictionary = diagnostics.get(
		"coil_animation_frame_counts",
		{}
	) as Dictionary
	for animation_name: StringName in REQUIRED_ESCAPE_SKIRMISH_ANIMATIONS:
		assert_int(
			int(spark_frame_counts.get(String(animation_name), 0))
		).is_greater_equal(MIN_CHARACTER_ANIMATION_FRAMES)
		assert_int(
			int(coil_frame_counts.get(String(animation_name), 0))
		).is_greater_equal(MIN_CHARACTER_ANIMATION_FRAMES)


func _assert_escape_skirmish_pacing_contract(diagnostics: Dictionary) -> void:
	var pacing: Dictionary = diagnostics.get("pacing", {}) as Dictionary
	assert_bool(bool(pacing.get("active", false))).is_true()
	assert_int(int(pacing.get("spark_opening_grace_frames", 0))).is_equal(
		ESCAPE_SPARK_RAT_OPENING_GRACE_FRAMES
	)
	assert_int(int(pacing.get("coil_opening_grace_frames", 0))).is_equal(
		ESCAPE_COIL_RAT_OPENING_GRACE_FRAMES
	)
