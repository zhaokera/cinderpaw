## Player Abilities Story 085: Old Factory lower-deck forward-pressure aftershock exit skirmish.
...
const EXIT_SPARK_RAT_OPENING_GRACE_FRAMES: int = 12
const EXIT_COIL_RAT_OPENING_GRACE_FRAMES: int = 24
...
const REQUIRED_EXIT_SKIRMISH_ANIMATIONS: Array[StringName] = [
	&"idle",
	&"run",
	&"attack_tell",
	&"attack",
...
func test_aftershock_exit_skirmish_requires_cache_claim_and_activates_dual_contract() -> void:
	var locked_scene: Node = _factory_scene_with_aftershock_exit_skirmish_state(
		false,
		false,
		false
	)
	...
	assert_bool(bool(locked.get("aftershock_reward_cache_claimed", true))).is_false()
	...
	var destination: Node = _factory_scene_with_aftershock_exit_skirmish_state(
		true,
		false,
		false
	)
	...
	assert_bool(bool(ready.get("aftershock_reward_cache_claimed", false))).is_true()
	...
	assert_int(int(active.get("spark_entity_id", 0))).is_equal(EXIT_SPARK_RAT_ENTITY_ID)
	assert_int(int(active.get("coil_entity_id", 0))).is_equal(EXIT_COIL_RAT_ENTITY_ID)
	...
	_assert_exit_skirmish_frame_contract(active)
	_assert_exit_skirmish_pacing_contract(active)


func test_aftershock_exit_skirmish_defeat_persists_without_replaying_route_chain() -> void:
	var destination: Node = _factory_scene_with_aftershock_exit_skirmish_state(
		true,
		false,
		false
	...
	assert_bool(destination.call("apply_damage", EXIT_SPARK_RAT_ENTITY_ID, 999, {
		"source": &"unit_test_aftershock_exit_skirmish_spark",
	})).is_true()
	...
	assert_bool(destination.call("apply_damage", EXIT_COIL_RAT_ENTITY_ID, 999, {
		"source": &"unit_test_aftershock_exit_skirmish_coil",
	})).is_true()
	await get_tree().process_frame
	...
	var restored: Node = _factory_scene_with_aftershock_exit_skirmish_state(
		true,
		true,
		true
	)
	...
	assert_bool(bool(restored_exit.get("present", false))).is_true()
	assert_bool(bool(restored_exit.get("aftershock_reward_cache_claimed", false))).is_true()
	assert_bool(bool(restored_exit.get("active", true))).is_false()
	assert_bool(bool(restored_exit.get("cleared", false))).is_true()
	assert_bool(bool(restored_exit.get("spark_visible", true))).is_false()
	assert_bool(bool(restored_exit.get("coil_visible", true))).is_false()
	assert_str(String(restored_exit.get("route_label_text", ""))).is_equal(
		"Forward Pressure Aftershock Exit Skirmish Cleared"
	)
	...
	var restored_aftershock: Dictionary = restored.call(
		"get_factory_lower_deck_forward_pressure_coil_aftershock_diagnostics"
	)
	assert_bool(bool(restored_aftershock.get("cleared", false))).is_true()
	...
	assert_bool(bool(old_forward_cache.get("claimed", false))).is_true()
...
func test_aftershock_exit_skirmish_unlocks_on_live_aftershock_cache_claim() -> void:
	var destination: Node = _factory_scene_with_aftershock_exit_skirmish_state(
		false,
		false,
		false
	...
	var before_exit: Dictionary = destination.call(
		"get_factory_lower_deck_forward_pressure_aftershock_exit_skirmish_diagnostics"
	)
	assert_bool(bool(before_exit.get("available", true))).is_false()
	assert_bool(bool(before_exit.get("active", true))).is_false()

	var before_cache: Dictionary = destination.call(
		"get_factory_lower_deck_forward_pressure_aftershock_reward_cache_diagnostics"
	)
	assert_bool(bool(before_cache.get("claim_available", false))).is_true()
	player.global_position = before_cache.get("position", Vector2.ZERO) as Vector2
	assert_bool(bool(destination.call(
		"try_claim_factory_lower_deck_forward_pressure_aftershock_reward_cache",
		player
	))).is_true()

	var after_claim_exit: Dictionary = destination.call(
		"get_factory_lower_deck_forward_pressure_aftershock_exit_skirmish_diagnostics"
	)
	assert_bool(bool(after_claim_exit.get("aftershock_reward_cache_claimed", false))).is_true()
	assert_bool(bool(after_claim_exit.get("available", false))).is_true()
	assert_bool(bool(after_claim_exit.get("active", true))).is_false()

	player.global_position.x = float(after_claim_exit.get("activation_x", 0.0)) + 4.0
	assert_bool(bool(destination.call(
		"try_activate_factory_lower_deck_forward_pressure_aftershock_exit_skirmish",
		player
	))).is_true()
...
func _factory_scene_with_aftershock_exit_skirmish_state(
		aftershock_reward_claimed: bool,
		spark_defeated: bool,
		coil_defeated: bool
) -> Node:
	...
	if destination == null:
		return null
	var skirmish_cleared: bool = spark_defeated and coil_defeated
	destination.call("set_local_state", _aftershock_exit_skirmish_base_state().merged({
		"factory_lower_deck_forward_pressure_aftershock_reward_cache_claimed": (
			aftershock_reward_claimed
		),
		"factory_lower_deck_forward_pressure_aftershock_exit_skirmish_activated": (
			skirmish_cleared
		),
...
func _aftershock_exit_skirmish_base_state() -> Dictionary:
	return {
...
func _assert_exit_skirmish_frame_contract(diagnostics: Dictionary) -> void:
	...
	for animation_name: StringName in REQUIRED_EXIT_SKIRMISH_ANIMATIONS:
...
func _assert_exit_skirmish_pacing_contract(diagnostics: Dictionary) -> void:
	...
	assert_int(int(spark_pacing.get("opening_grace_frames", 0))).is_equal(
		EXIT_SPARK_RAT_OPENING_GRACE_FRAMES
	)
	assert_int(int(coil_pacing.get("opening_grace_frames", 0))).is_equal(
		EXIT_COIL_RAT_OPENING_GRACE_FRAMES
	)
