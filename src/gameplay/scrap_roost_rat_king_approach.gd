## Post-onboarding ACT encounter before the Rat King arena.
extends Node2D

const APPROACH_SCENE_ID: StringName = &"area_01_scrap_roost_rat_king_approach"
const MAIN_SCENE_ID: StringName = &"main"
const MAIN_SPAWN_POINT: StringName = &"scrap_roost"
const ENEMY_ENTITY_ID: int = 2
const ENEMY_ACTIVATION_X: float = 440.0
const PLAYER_SPAWN_POSITION: Vector2 = Vector2(140.0, 456.0)
const ENEMY_SPAWN_POSITION: Vector2 = Vector2(820.0, 456.0)
...
const SHADOW_BEAST_SCENE: PackedScene = preload("res://src/gameplay/simple_enemy.tscn")
...
@onready var _exit_seal: Sprite2D = $ExitSeal
@onready var _exit_pulse: Sprite2D = $ExitSeal/ExitPulse

var _scene_manager: Object = null
var _weapon_component: WeaponComponent = null
var _damage_adapter: Object = null
var _enemy: SimpleEnemy = null
...
func _physics_process(delta: float) -> void:
	...
	if _player.global_position.y > FALL_RESET_Y:
		_reset_attempt()
		return
	if not _enemy_active and not _enemy_defeated \
			and _player.global_position.x >= ENEMY_ACTIVATION_X:
		_activate_enemy()
...
func apply_damage(target_id: int, final_damage: int, metadata: Dictionary = {}) -> bool:
	if (
		...
		or _enemy.get_entity_id() != target_id
	):
		return false
	_enemy.apply_damage(final_damage, metadata)
	return true


func get_approach_diagnostics() -> Dictionary:
	...
	return {
		"scene_id": String(APPROACH_SCENE_ID),
		"enemy_active": _enemy_active,
		"enemy_defeated": _enemy_defeated,
		"enemy_present": is_instance_valid(_enemy),
		"enemy_hp": _enemy.get_current_hp() if is_instance_valid(_enemy) else 0,
		...
		"idle_frames": _get_animation_frame_count(enemy_frames, &"idle"),
		"patrol_frames": _get_animation_frame_count(enemy_frames, &"patrol"),
		"attack_tell_frames": _get_animation_frame_count(enemy_frames, &"attack_tell"),
		"attack_frames": _get_animation_frame_count(enemy_frames, &"attack"),
		"hurt_frames": _get_animation_frame_count(enemy_frames, &"hurt"),
		"death_frames": _get_animation_frame_count(enemy_frames, &"death"),
...
func _spawn_enemy() -> void:
	_enemy = SHADOW_BEAST_SCENE.instantiate() as SimpleEnemy
	if _enemy == null:
		return
	_enemy.name = "ApproachShadowBeast"
	_enemy.position = ENEMY_SPAWN_POSITION
	add_child(_enemy)
	_enemy.set_damage_calculator_adapter(_damage_adapter)
	_enemy.set_attack_target(null)
...
func _on_enemy_defeated() -> void:
	var defeated_position: Vector2 = (
		_enemy.global_position if is_instance_valid(_enemy) else ENEMY_SPAWN_POSITION
	)
	_enemy_active = false
	_enemy_defeated = true
	_exit_unlocked = true
	_sync_exit_gate()
	_combat_presentation.on_kill_event(ENEMY_ENTITY_ID, defeated_position)
...
func _request_main_handoff() -> bool:
	...
	var accepted: bool = bool(
		_scene_manager.call("request_scene_change", MAIN_SCENE_ID, MAIN_SPAWN_POINT)
	)
...
func _sync_exit_gate() -> void:
	...
	_exit_seal.visible = not _exit_unlocked
...
func _update_exit_pulse() -> void:
	if not _exit_pulse.visible:
		return
	var pulse_alpha: float = 0.12 + 0.12 * (0.5 + 0.5 * sin(_elapsed_sec * 5.0))
...
func _get_animation_frame_count(frames: SpriteFrames, animation_name: StringName) -> int:
