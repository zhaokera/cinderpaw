## Data-driven Central Tower lift sentry with a telegraphed ram attack.
class_name CentralTowerCounterweightSentry
extends "res://src/gameplay/neon_signal_rat.gd"

const SENTRY_CONFIG_ID: StringName = &"central_tower_counterweight_sentry"
const SENTRY_ENTITY_FAMILY: StringName = &"central_tower_counterweight_sentry"
const SENTRY_ATTACK_HITBOX_ID: StringName = (
	&"central_tower_counterweight_sentry_ram"
)
const SENTRY_PATTERN_ID: StringName = &"counterweight_ram"
const SENTRY_ATTACK_HIT_FRAME: int = 143
const SENTRY_HURTBOX_SIZE: Vector2 = Vector2(44.0, 68.0)
const SENTRY_DEFAULT_MAX_HP: int = 44
const SENTRY_DEFAULT_PATROL_SPEED: float = 56.0
const SENTRY_DEFAULT_STARTUP_FRAMES: int = 24
const SENTRY_DEFAULT_ACTIVE_FRAMES: int = 5
const SENTRY_DEFAULT_RECOVERY_FRAMES: int = 24
const SENTRY_DEFAULT_DAMAGE: int = 12
const SENTRY_DEFAULT_LUNGE_SPEED: float = 120.0
const SENTRY_DEFAULT_ATTACK_RANGE_PX: float = 108.0
const SENTRY_DEFAULT_HITBOX_SIZE: Vector2 = Vector2(70.0, 42.0)
const SENTRY_DEFAULT_HITBOX_OFFSET: Vector2 = Vector2(44.0, -32.0)


func get_enemy_family_id() -> StringName:
	return SENTRY_ENTITY_FAMILY


func get_config_diagnostics() -> Dictionary:
	return {
		"config_id": String(SENTRY_CONFIG_ID),
...
## Restores a failed lift attempt without rebuilding the Tower scene.
...
func _setup_core_components() -> void:
	...
	_collision.set_hurtbox_size(SENTRY_HURTBOX_SIZE)
...
func _build_attack_metadata() -> Dictionary:
	return {
		"source": SENTRY_ENTITY_FAMILY,
		"owner_boss_id": get_summon_owner_boss_id(),
		"summon_id": get_current_summon_id(),
		"attack_type": &"light",
		"weapon_id": SENTRY_ATTACK_HITBOX_ID,
		"combo_index": 0,
		"hit_frame": SENTRY_ATTACK_HIT_FRAME,
		"attack_power": 0,
		"enemy_defense": 0,
		"active_frames": _configured_active_frames,
		"recovery_frames": _configured_recovery_frames,
		"lunge_speed": _configured_lunge_speed,
		"config_id": SENTRY_CONFIG_ID,
...
func _build_enemy_damage_params() -> Dictionary:
	return {
		"entries": {
			String(SENTRY_ATTACK_HITBOX_ID): {
...
func _load_enemy_config() -> void:
	_configured_max_hp = SENTRY_DEFAULT_MAX_HP
	_configured_patrol_speed = SENTRY_DEFAULT_PATROL_SPEED
	_configured_startup_frames = SENTRY_DEFAULT_STARTUP_FRAMES
	_configured_active_frames = SENTRY_DEFAULT_ACTIVE_FRAMES
	_configured_recovery_frames = SENTRY_DEFAULT_RECOVERY_FRAMES
	_configured_damage = SENTRY_DEFAULT_DAMAGE
	_configured_lunge_speed = SENTRY_DEFAULT_LUNGE_SPEED
	_configured_attack_range_px = SENTRY_DEFAULT_ATTACK_RANGE_PX
	_configured_hitbox_size = SENTRY_DEFAULT_HITBOX_SIZE
	_configured_hitbox_offset = SENTRY_DEFAULT_HITBOX_OFFSET
	...
	_configured_max_hp = maxi(
		1,
		int(entry.get("max_hp", SENTRY_DEFAULT_MAX_HP))
	)
	_configured_patrol_speed = maxf(
		0.0,
		float(entry.get("patrol_speed", SENTRY_DEFAULT_PATROL_SPEED))
	)
