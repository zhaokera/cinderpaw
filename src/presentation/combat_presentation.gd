...
func on_kill_event(_target_id: int, position: Vector2) -> void:
	play_hitstop(KILL_HITSTOP_FRAMES)
	play_screen_shake(KILL_SHAKE_INTENSITY, KILL_HITSTOP_FRAMES)
	_spawn_debris(position, KILL_DEBRIS_COUNT)
...
func _spawn_damage_number(position: Vector2, damage: int, color: Color) -> void:
	_last_damage_number_text = str(damage)
	_last_damage_number_color = color

	var label := Label.new()
	label.text = _last_damage_number_text
	label.position = position + Vector2(-12, -42)
...
func _spawn_sparks(position: Vector2, count: int, color: Color) -> void:
	for index: int in range(maxi(0, count)):
		...
		spark.position = position + Vector2(float((index % 4) * 8 - 12), float((index / 4) * 7 - 10))
		spark.color = color
		spark.rotation = float(index) * 0.5
		spark.z_index = 80
		add_child(spark)
		var tween: Tween = create_tween()
		tween.tween_property(spark, "position", spark.position + Vector2(float(index - count / 2) * 4.0, -18.0), SPARK_LIFETIME_SEC)
...
func _spawn_debris(position: Vector2, count: int) -> void:
	for index: int in range(maxi(0, count)):
		var shard := ColorRect.new()
		shard.size = Vector2(5, 5)
		shard.position = position + Vector2(float((index % 6) * 7 - 20), float((index / 6) * 7 - 12))
		shard.color = DEBRIS_COLOR
		shard.rotation = float(index) * 0.7
		shard.z_index = 82
		add_child(shard)
		var tween: Tween = create_tween()
		tween.tween_property(shard, "position", shard.position + Vector2(float(index - count / 2) * 3.0, 18.0), DEBRIS_LIFETIME_SEC)
