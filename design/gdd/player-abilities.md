...
| dodge | 闪避 | 游戏开始 | 主动 |
| dash | 冲刺 | 击败垃圾桶鼠王 | 主动 |
| double_jump | 二段跳 | 击败第2个Boss **或** 发现隐藏Boss | 主动 |
| aerial_attack | 下劈 | 击败第3个Boss | 主动 |
| wall_climb | 墙壁攀爬 | 击败第4个Boss **或** 发现隐藏祭坛 | 主动 |
| parry | 弹反 | 教学关卡 | 主动 |

**探索能力路径说明**：二段跳和墙壁攀爬提供替代获取路径，增加银河城式非线性探索。玩家可以选择：
- **主线路径**：按顺序击败Boss获取能力（线性但节奏可控）
- **探索路径**：通过发现隐藏区域提前获取能力（非线性，奖励探索）

隐藏Boss位于废弃商业街的下水道入口后方（需要发现秘密墙壁），隐藏祭坛位于旧工厂的顶部平台（需要精确跳跃）。探索路径获取的能力与Boss路径完全相同，确保玩家无论选择哪条路径都能获得相同的能力集合。
...
**上游依赖**：Boss配置层、生命检测
**下游被依赖**：探索与能力门控、猫科战斗、战斗表现、HUD/UI、护符/装备系统（能力解锁影响护符效果条件）
