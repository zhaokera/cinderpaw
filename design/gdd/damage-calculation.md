...
**武器基础伤害差距说明**：武器 base_damage 差距为 5×（猫爪 10 → 鱼骨大剑 50），这是有意设计。设计意图：
1. **武器选择有意义**：高 base 武器适合新手或保守玩法，低 base 武器适合技巧型玩家
2. **技巧差距在高端对局体现**：虽然菜鸟用鱼骨普通攻击 ≈ 高手用猫爪连招暴击，但高手可以通过 PERFECT 弹反（5.0×）和 combo 优化（2.5×1.8×1.2）达到更高 DPS
3. **武器特色补偿**：高 base 武器有特殊机制（鱼骨蓄力 2.0×、范围攻击），低 base 武器有速度优势（猫爪攻速快、闪避后反击窗口+3帧）

验证表（defense=30, reduction=0.667）：
| 操作 | 猫爪(base=10) | 鱼骨(base=50) | 差距 |
|------|:---:|:---:|:---:|
| 普通攻击 | 6 | 33 | 5.5× |
| 连招终结+暴击 | 30 | 183 | 6.1× |
| PERFECT弹反+暴击 | 83 | 416 | 5.0× |

结论：武器选择影响玩法风格，但技巧玩家可通过弹反和 combo 优化缩小差距。如 playtest 显示差距过大，优先调整鱼骨蓄力时间或猫爪反击倍率。
