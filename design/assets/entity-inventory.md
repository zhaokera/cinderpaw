...
| 5 | Rat King Boss | Boss | First boss visual slice now has image-generated `AnimatedSprite2D + SpriteFrames` art for `idle`, `attack_tell`, `attack`, `hurt`, `death`, `phase_1_intro`, `phase_2_rebuild`, and `phase_3_overload`, with transparent 192x192 frames under `assets/characters/rat_king/`. Full runtime BossConfig/AI scene integration and the remaining specialized attacks (`charge`, `claw_attack`, `summon`, `jump_slam`, `combo_attack`) remain pending. | design/gdd/boss-config.md; design/art/art-bible.md; production/epics/combat-presentation/story-014-rat-king-boss-frame-animation.md | Partial |
