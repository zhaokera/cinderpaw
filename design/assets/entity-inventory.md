...
| 29 | Old Factory Deep Route Endpoint | Environment / Interactive Route Objective | Deep-route checkpoint switch with an image-generated transparent rusted pedestal, cat-paw mechanical switch, amber cat-eye indicator, and scene-local once-only activation state. Story010 mounts it in `factory_route_transition_shell.tscn`; it starts locked while `FactoryDeepGuardRatMinion` is alive, becomes activable after that second Rat Minion is defeated, marks `factory_deep_route_cleared`, and persists through `OldFactoryEntranceScene.get_local_state()` / `set_local_state()`. | design/gdd/player-abilities.md; design/gdd/exploration-ability-gating.md; design/gdd/scene-management.md; production/epics/player-abilities/story-010-old-factory-deep-route-micro-slice.md | Implemented baseline |
