...
| 30 | Factory Spark Rat | Enemy / Old Factory Patrol | Distinct Old Factory enemy family with image-generated `AnimatedSprite2D + SpriteFrames` art for `idle`, `run`, `attack`, `hurt`, and `death`. Story013 mounts it in `factory_route_transition_shell.tscn` as `FactorySparkRat` with entity id `2102`; it starts visible but inactive, activates after the deep-route endpoint opens, can be damaged through the player attack adapter, and persists defeat through scene-local state. | design/gdd/player-abilities.md; design/gdd/exploration-ability-gating.md; design/gdd/feline-combat.md; production/epics/player-abilities/story-013-old-factory-spark-rat-patrol-encounter.md | Implemented baseline |
